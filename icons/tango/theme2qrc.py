...
<!DOCTYPE RCC><RCC version="1.0">
  <qresource prefix="icons/tango">
    ...
    <file>16x16/actions/document-new.png</file>
    <file>16x16/actions/document-open.png</file>
    ...
    <file>scalable/places/flag-es.svg</file>
    <file>scalable/places/flag-en.svg</file>
  </qresource>
</RCC>
"""
...
for line in data:
    ...
    if line.startswith('[') and line.endswith(']'):
        if line[1:-1] not in theme:
            theme[line[1:-1]] = {}
            section = line[1:-1]
    elif line.count('=') == 1:
        cat, val = line.split('=')
        theme[section][cat] = val
        
directories = [a.split('/') for a in theme["Icon Theme"]["Directories"].split(',')]

print directories

out = file('parsed.qrc','w')


out.write('<!DOCTYPE RCC><RCC version="1.0">' + '\n')
out.write('   ' + '<qresource prefix="icons/tango">' + '\n')
out.write('   ' + '   ' + '<file>index.theme</file>' + '\n')
#dir_list = os.listdir('mydir')

for directory in directories:
    d1,d2 = directory
    dir_list = [x for x in  os.listdir(d1 + '/' + d2) if x.endswith('.png') or x.endswith('.svg')]
    for icon in dir_list:
        out.write('   ' + '   ' + '<file>' + d1 + '/' + d2 + '/' + icon + '</file>' + '\n')
out.write('   ' + '</qresource>' + '\n')
out.write('</RCC>')
"""
[Icon Theme]
